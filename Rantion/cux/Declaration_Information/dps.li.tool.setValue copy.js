 ...
 * @LastEditTime   : 2020-05-26 19:17:41
 * @LastEditors    : Li
 * @Description    :  用于渲染表格
 * @FilePath       : \Rantion\cux\Declaration_Information\dps.li.tool.setValue copy.js
...
define(['N/file', 'N/search', 'N/record', 'N/log', './handlebars-v4.1.1',
    ...
    function setModuleXMLValue(INV, xmlID) {
        ...
        printData = setBoxValue(BOX, xmlID, printData);
        printData = setDeclarationValue(inv, xmlID, printData);
        printData = setContractValue(con, xmlID, printData);
    ...
    function setInvValue(INV, xmlID, printData) {
        ...
        var fileObj = file.create({
            name: "报关发票" + inv_number + '_' + nowDate + ".xls",
    ...
    function setBoxValue(BOX, xmlID, printData) {
        ...
        // 获取模板内容,写全路径或者内部ID  "SuiteScripts/printPO/print_contract/contract.xml"
        var model = file.load({
            id: xmlID
        }).getContents();
        ...
        nowDate = nowDate.toISOString();
        var fileObj = file.create({
            name: "报关装箱" + inv_no + '_' + nowDate + ".xls",
            fileType: file.Type.EXCEL,
            contents: encode.convert({
                string: xml_1,
                inputEncoding: encode.Encoding.UTF_8,
    ...
    function setDeclarationValue(inv, xmlID, printData) {
        ...
        log.debug('setDeclarationValue printData', printData);
        ...
        nowDate = nowDate.toISOString();
        var fileObj = file.create({
            name: "报关单-" + printData['contra'] + '-' + nowDate + ".xls",
            fileType: file.Type.EXCEL,
            contents: encode.convert({
                string: xml_1,
                inputEncoding: encode.Encoding.UTF_8,
    ...
    function setContractValue(con, xmlID, printData) {
        ...
        nowDate = nowDate.toISOString();
        var fileObj = file.create({
            name: "报关合同-" + printData['con'] + '-' + nowDate + ".xls",
            fileType: file.Type.EXCEL,
            contents: encode.convert({
                string: xml_1,
                inputEncoding: encode.Encoding.UTF_8,
    ...
    function setElemValue(inv, xmlID, printData) {
        ...
        nowDate = nowDate.toISOString();
        var fileObj = file.create({
            name: "申报要素-" + inv_number + '-' + nowDate + ".xls",
            fileType: file.Type.EXCEL,
            contents: encode.convert({
                string: xml_1,
                inputEncoding: encode.Encoding.UTF_8,
    ...
    return {
        ...
        setDeclarationValue: setDeclarationValue,
        setContractValue: setContractValue,
        setElemValue: setElemValue,
        setUSValue: setUSValue
