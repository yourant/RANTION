...
define(['N/search', 'N/record', 'N/log', 'N/format'], function(search, record, log, format) {
        ...
        try{
            ...
            search.create({
                ...
                columns: [
                    ...
                    'custrecord_rsf_sales_forcast_alter'
                ]
            }).run().each(function (rec) {
                data.push({
                    forcast_store: rec.getValue('custrecord_rsf_sales_forcast_store'),
                    forcast_item: rec.getValue('custrecord_rsf_sales_forcast_item'),
                    ...
                    forcast_alter: rec.getValue('custrecord_rsf_sales_forcast_alter')
                });
                return --limit > 0;
            });
            log.debug('data',data);
    ...
    function map(context) {
        ...
        try {
            var monthStartDate = format.parse({ value:obj.forcast_date, type: format.Type.DATE});
            //log.debug('monthStartDate', monthStartDate);
            var year = monthStartDate.getFullYear();
            var month = monthStartDate.getMonth() + 1;
            var now_date = new Date();
            var now_year = now_date.getFullYear();
            var store = obj.forcast_store;
            var item = obj.forcast_item;
            var monthQty = Number(obj.forcast_alter) > 0 ? Number(obj.forcast_alter) : Number(obj.forcast_quantity);
            var monthWeekDate = splitMonthToWeek(monthStartDate, monthQty);
            //log.debug('forcast_store', store);
            //log.debug('forcast_item', item);
            if(store == 56 && item == 13440){
