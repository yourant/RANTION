...
define(['N/search', 'N/ui/serverWidget','../../Helper/Moment.min', 'N/format', 'N/runtime', 'N/record'], function(search, ui, moment, format, runtime, record) {
    ...
    */
    function initUI() {
        var form = ui.createForm({ title: '交货计划例外信息查询' });
    ...
    */
    function getResult(item, date_from, date_to, pageSize, nowPage,site,account) {
        ...
        //计划交货量  ，确认交货量
        var filters = [
            { name : 'custrecord_demand_forecast_l_data_type', join:'custrecord_demand_forecast_parent', operator:'anyof', values: ["14","16"] },
...
if (totalCount == 0 && pageCount ==0) {
    ...
    */
    function createLineData(form, result, date_from, date_to) {
        ...
        var sublist = form.addSublist({ id: 'custpage_sublist', type: ui.SublistType.LIST, label: '交货计划例外信息', tab: 'custpage_tab' });
