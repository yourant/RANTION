...
define(['N/encode', 'SuiteScripts/douples_amazon/Helper/handlebars-v4.1.1', 'N/file', 'N/runtime', 'SuiteScripts/douples_amazon/Helper/Moment.min', 'N/search', 'N/format'
 ...
 /**
   ...
   function getResult_Replen (item, week_rs, func_type, account) {
    ...
    var item_data = []
     //搜索店铺净需求，确认交货
    item_data = GetPredictionData(item_data, ['23'], today, account, skuids,SKUIds, {data_type: '23',data_type_text: '店铺净需求'},week_rs,func_type); 
    //确认交货量
    item_data = GetPredictionData(item_data, ['16'], today, account, skuids,SKUIds, {data_type: '16',data_type_text: '确认交货量'},week_rs,func_type); 
  //倒排补货量
  var item_arr = []
  search.create({
      type: 'item',
  ...
  /**
   ...
   function getResult_Delivery (item, week_rs, func_type, account) {
    ...
    var item_data = []
    // 调拨计划量  ，取自修改的调拨计划量 22
    item_data = GetPredictionData(item_data, ['22'], today, account, skuids,SKUIds, {data_type: '6',data_type_text: '调拨计划量'},week_rs,func_type); 
        var location=[];
        //获取所有自营仓
        search.create({
            type: 'location',
    ...
    //修改就货量
    item_data = GetPredictionData(item_data, ['15'], today, account, skuids,SKUIds, {data_type: '8',data_type_text: '修改交货量'},week_rs,func_type); 
    //确认交货量
    item_data = GetPredictionData(item_data, ['16'], today, account, skuids,SKUIds, {data_type: '9',data_type_text: '确认交货量'},week_rs,func_type); 
  ...
  function getResult_Alloca (item, week_rs, func_type, account) {
    ...
    item_data = GetPredictionData(item_data, ['11'], today, account, skuids ,SKUIds, {data_type: '11',data_type_text: '调拨在途量'},week_rs,func_type); // 11在途量
    item_data = GetPredictionData(item_data, ['3'], today, account, skuids ,SKUIds, {data_type: '3',data_type_text: '店铺净需求'},week_rs,func_type); // 3 净需求
    ...
    item_data = GetPredictionData(item_data, ['22'], today, account, skuids, SKUIds, {data_type: '6',data_type_text: '修改调拨计划量'},week_rs,func_type);
  ...
  function getResult_Demand (item, week_rs, func_type, account) {
    ...
    SKUIds.map(function (line) {
     ...
     // 23 修改净需求量
     item_data=GetPredictionData (item_data, "23", today, account, skuids, SKUIds, {data_type: '23',data_type_text: '修改净需求量'},week_rs,func_type);
