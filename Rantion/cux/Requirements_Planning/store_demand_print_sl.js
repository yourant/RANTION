...
define(['N/encode', 'SuiteScripts/douples_amazon/Helper/handlebars-v4.1.1', 'N/file', 'N/runtime', 'SuiteScripts/douples_amazon/Helper/Moment.min', 'N/search', 'N/format'
  ...
  var week_rs,func_type,sku_arrys = [], account_arrs = [],acc_skus = {}
  var skucs = []
  function onRequest (context) {
    ...
    var lineComObj
    var fileName,filePage = fils.nowPage
    if (fils.TT == 'Demand') { // 店铺供应需求
      fileName = '店铺供需'
      // getResult_Demand (item, week_rs, func_type, account,nowPage,pageSize) 
      lineComObj = getResult_Demand(fils.sku, fils.week_rs, fils.func_type, fils.acc, fils.nowPage, fils.pageSize, fils.year)
    }else if (fils.TT == 'Alloca') { // 调拨计划
      fileName = '调拨计划'
      lineComObj = getResult_Alloca(fils.sku, fils.week_rs, fils.func_type, fils.acc, fils.nowPage, fils.pageSize, fils.year)
    }else if (fils.TT == 'Delivery') { // 交货计划
      fileName = '交货计划'
      lineComObj = getResult_Delivery(fils.sku, fils.week_rs, fils.func_type, fils.acc, fils.nowPage, fils.pageSize, fils.year)
    }else if (fils.TT == 'Replen') { // 补货计划
      fileName = '补货计划'
      lineComObj = getResult_Replen(fils.sku, fils.week_rs, fils.func_type, fils.acc, fils.nowPage, fils.pageSize, fils.year)
    }
  ...
  * @param {*} date_to 
  */
  function getResult_Demand (item, week_rs, func_type, account, nowPage, pageSize, year) {
    log.debug('传进来的nowPage ', nowPage)
    ...
    if (totalCount == 0 && pageCount == 0) {
      ...
      pageData_demand_forecast.fetch({
        ...
        account_arrs.push(rs.getValue(rs.columns[0]))
    ...
    if (need_transit_num.length > 0) {
      for (var a = 0; a < need_transit_num.length; a++) {
        for (var i = 0; i < skucs.length; i++) {
          if (skucs[i]['item_sku'] == need_transit_num[a]['item_id']) {
            item_data.push({
              item_sku: skucs[i]['item_sku'],
              item_sku_text: skucs[i]['item_sku_name'],
              item_name: skucs[i]['item_name'],
              account: skucs[i]['forecast_account'],
              account_text: skucs[i]['forecast_account_name'],
              site: skucs[i]['forecast_site'],
              item_leve: skucs[i]['item_leve'],
              itemf_leve: skucs[i]['itemf_leve'],
              data_type: '2',
              data_type_text: '调拨在途量',
              transit_no: need_transit_num[a]['lineItems']
            })
          }else {
            if (po_no.indexOf(skucs[i]['item_sku']) == -1) {
              item_data.push({
                item_sku: skucs[i]['item_sku'],
                item_sku_text: skucs[i]['item_sku_name'],
                item_name: skucs[i]['item_name'],
      ...
      skucs.map(function (line) {
        if (JSON.stringify(sku_arrys).indexOf(line.item_sku) > -1) {
          item_data.push({
    ...
    if (location_quantity.length > 0) {
      for (var i = 0; i < skucs.length; i++) {
        for (var a = 0; a < location_quantity.length; a++) {
          if (skucs[i]['item_sku'] == location_quantity[a]['item_id']) {
            item_data.push({
              item_sku: skucs[i]['item_sku'],
              item_sku_text: skucs[i]['item_sku_name'],
              item_name: skucs[i]['item_name'],
            ...
            if (temporary_arr.indexOf(skucs[i]['item_sku']) == -1) {
              item_data.push({
                item_sku: skucs[i]['item_sku'],
                item_sku_text: skucs[i]['item_sku_name'],
                item_name: skucs[i]['item_name'],
              ...
              temporary_arr.push(skucs[i]['item_sku'])
            }
          }
        }
      }
    ...
    // 23 修改净需求量
    item_data = GetPredictionData(item_data, '23', today, account_arrs, sku_arrys, skucs, {data_type: '23',data_type_text: '修改净需求量'}, week_rs, nowPage, acc_skus, pageSize)
              ...
              week_rs.map(function (wek) {
                ...
                if (wek == week_rs[0]) {
                  if (result[a]['location_no']) {
                    ...
                    log.debug("库存的result[a]['location_no']", result[a])
                  }else {
                    need_no = 0
                    weekArray.push({week: week,qty: '0'})
                  }
                  ...
                  var x1 = need_no
                  log.debug("库存的[need_no]['x1']", need_no)
                  var x2
                  if (need2_zl || need2_zl == 0) {
                    if (result[need2_zl]['transit_no'] == 0) {
                      x2 = 0
                  ...
                  if (!x2) x2 = 0
                  log.debug('result[need1_zl]', result[need1_zl])
                  var x3 = need1_zl || need1_zl == 0 ? result[need1_zl]['quantity_week' + wek] ? result[need1_zl]['quantity_week' + wek].split(' ')[1] : 0 : 0; // 需求量
                  need_no = -(Number(x3) - (Number(x1) + Number(x2)))
                  log.debug('x3: ' + x3, 'x1: ' + x1 + ',x2: ' + x2)
  ...
  * @param {*} func_type 
  */
  function getResult_Alloca (item, week_rs, func_type, account, nowPage, pageSize, year) {
    var dateFormat = runtime.getCurrentUser().getPreference('DATEFORMAT')
    var today = moment(new Date(+new Date() + 8 * 3600 * 1000).getTime()).format(dateFormat)
    ...
    item_data = GetPredictionData(item_data, ['23'], today, account, skuids , SKUIds, {data_type: '3',data_type_text: '店铺净需求'}, week_rs, nowPage, acc_skus, pageSize); // 3 净需求
            ...
            if (result[a]['data_type'] == 3) { // 店铺净需求量
              ...
              week_rs.map(function (wek) {
                ...
                if (func_type == 'B' && wek < week_rs[0]) // 跨年，明年的第一周不要
                  ff = wek - 1
                week = 'W' + ff
                log.debug('data_arr[need1_zl]', data_arr[need1_zl])
                log.debug('data_arr[need2_zl]', data_arr[need2_zl])
                log.debug('data_arr[need3_zl]', data_arr[need3_zl])
                var x1 = data_arr[need1_zl]['weekArray'][wek]['qty']
                ; // 需求预测 
                var x2 = data_arr[need2_zl]['weekArray'][wek]['qty']
                ; // 库存量
                var x3 = data_arr[need3_zl]['weekArray'][wek]['qty']
                ; // 在途量
                var x4 = Math.round(x1 - x2 - x3)
                sublist.setSublistValue({ id: sub_filed, value: Math.round(x4) ? Math.round(x4).toFixed(0) : '0', line: zl})
                weekArray.push({week: week,qty: Math.round(x4) ? Math.round(x4).toFixed(0) : '0'})
              })
              need4_zl = zl
            }
  ...
  /**
     ...
     * @param {*} func_type 
     */
  function getResult_Replen (item, week_rs, func_type, account, nowPage, pageSize, year) {
    var dateFormat = runtime.getCurrentUser().getPreference('DATEFORMAT')
    var today = moment(new Date(+new Date() + 8 * 3600 * 1000).getTime()).format(dateFormat)
    var SKUIds = [], limit = 4000
    var filters_sku = [],skuids = [],location = []
    ...
    // 确认交货量
    item_data = GetPredictionData(item_data, ['16'], today, account, sku_arrys, SKUIds, {data_type: '16',data_type_text: '确认交货量'}, week_rs, nowPage, acc_skus, pageSize)
  ...
  /**
    ...
    * @param {*} func_type 
    */
  function getResult_Delivery (item, week_rs, func_type, account, nowPage, pageSize, year) {
    var dateFormat = runtime.getCurrentUser().getPreference('DATEFORMAT')
    var today = moment(new Date(+new Date() + 8 * 3600 * 1000).getTime()).format(dateFormat)
    var SKUIds = [], limit = 4000
    var filters_sku = [],skuids = [],location = []
    ...
    item_data = GetInventoryData(skucs, warehouse_stock1, item_data, temporary_arr, '5', '美西仓库存')
    ...
    item_data = GetInventoryData(skucs, warehouse_stock2, item_data, temporary_arr, '111', '其他自营仓与工厂仓库存')
    ...
    // 修改就货量
    item_data = GetPredictionData(item_data, ['15'], today, account, skuids, skucs, {data_type: '8',data_type_text: '修改交货量'}, week_rs, nowPage, skuids, pageSize)
    // 确认交货量
    item_data = GetPredictionData(item_data, ['16'], today, account, skuids, skucs, {data_type: '9',data_type_text: '确认交货量'}, week_rs, nowPage, skuids, pageSize)
  ...
  * 搜索销售预测数据
  */
  function GetPredictionData (item_data, datatype, today, account, skuids, SKUIds, params, week_rs, nowPage, acc_skus, pageSize) {
    log.debug('传进方法的nowPage', nowPage)
    var filters = [
    ...
    if (totalCount == 0 && pageCount == 0) {
      ...
      log.debug('nowPage:' + JSON.stringify(nowPage), 'Number(nowPage - 1 ):' + Number(nowPage - 1))
      pageData_demand_forecast.fetch({
        index: Number(nowPage - 1)
      }).data.forEach(function (rs) {
        for (var key_acc in acc_skus) {
