...
define(['N/search', 'N/ui/serverWidget', 'N/format'],
    function (search, serverWidget, format) {
        ...
        function showResult(method, params, response) {
            var form = initUI(); //渲染查询条件UI界面，但“数据选择”无选项
        ...
        function getSearchData(form, params) {
            ...
            // var po_s = params.custpage_po_s; // po采购单
        ...
        function getResult(pageSize, nowPage, vendor_s, start_date_s, end_date_s) {
            ...
            filters.push({
                name: 'custbody_dps_invoice_status',
                operator: 'noneof',
                values: 3
            });
            // filters.push({
            //     name: 'custbody_dps_undelivered_quantity',
            //     operator: 'greaterthan',
            //     values: ['0']
            // });
            //如果需要限定状态为“待收货”订单，加上status为pendingReceipt的filters
            filters.push({
         ...
         */
        function initUI() {
            var form = serverWidget.createForm('批量交货单处理');
            form.addFieldGroup({
            ...
            sku_sublist.addField({
                id: 'custpage_sub_po_vendor_no',
                type: serverWidget.FieldType.TEXT,
                label: '供应商编号'
            });
            sku_sublist.addField({
                id: 'custpage_sub_po_employee',
                type: serverWidget.FieldType.SELECT,
                source: 'employee',
                label: '采购员'
            }).updateDisplayType({
            ...
            form.addButton({
                ...
                label: '生成批量交货单',
                functionName: 'createLpn()'
            }); //点击生成使用对应cs脚本
            form.clientScriptModulePath = './pln_manage_create_cs.js';
            return form;
        }
