...
define(['N/search', 'N/record'], function (search, record) {
    ...
    function afterSubmit(context) {
        ...
        log.debug('so_fulfill_ue', JSON.stringify(context));
        if (type == 'create' && subsidiary != '5') {
            ...
            if (recordType == 'salesorder') {
                var so_id = newRec.getValue('createdfrom');
                ...
                if (item_ids.length > 0 && location) {
                    ...
                    if (po_ids && po_ids.length > 0) {
                        ...
                        linkSoData2Po(poRecInfo, poNeedCount, itemJson, so_id);
                    }
                }
            }
        }
    ...
    /**
     * 将SO单上的货品数量占用采购收货单上的货品
     * @param {*} poRecInfo
     * @param {*} itemJson
     */
    function linkSoData2Po(poRecInfo, poNeedCount, itemJson, SoId) {
        var poNeedSaveJson = {};
            ...
            for (var i in poRecInfo[key]) {
                ...
                if (needCount && needCount > 0) {
                    ...
                    if (ToCount && ToCount > 0) {
                        ...
                        log.debug('createSoLinkRecord', poRec.getValue('tranid') + '==' + poData.line + '==' + actuallyCount + '==' + poData.link + '==' + SoId);
                        poNeedSaveJson[po_id] = createSoLinkRecord(poRec, poData.line, actuallyCount, poData.link, SoId, poNeedCount, needCountkey);
                    }
                }
            }
        }
        for (var po_id in poNeedSaveJson) {
            ...
            try {
                ...
                log.debug('salesorder_po', JSON.stringify(error));
            }
        }
        //开始计算用掉了多少数量，然后再更新TO货品行数据
        refreshItemCount(itemJson, SoId);
    }
     ...
     * @param {*} SoId
     */
    function createSoLinkRecord(poRec, itemReceiptLine, count, linkId, SoId, poNeedCount, needCountkey) {
        ...
        //之前没有未占完的情况
            ...
            linkRec.setValue({ fieldId: 'name', value: 'PO:' + poRec.getValue('tranid') + ':' + (itemReceiptLine + 1) + ':(SO)' });
            linkRec.setSublistValue({
                sublistId: 'recmachcustrecord__realted_transfer_head',
                fieldId: 'custrecord_transfer_code',
                value: SoId,
    ...
    //计算并更新未占用的数量
    function refreshItemCount(itemJson, SoId) {
        var soRec = record.load({ type: 'salesorder', id: SoId });
        var line = soRec.getLineCount({ sublistId: 'item' });
        var hasChange;
        for (var key in itemJson) {
            //占用完后剩余的数量
            ...
            for (var i = 0; i < line; i++) {
                ...
                if (item == key) {
                    var quantity = Number(soRec.getSublistValue({ sublistId: 'item', fieldId: 'quantity', line: i }));
                    if (quantity) {
