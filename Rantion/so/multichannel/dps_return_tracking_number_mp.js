...
function(log, search, record, encode, https, xml) {
    ...
    function getInputData() {
        ...
        try{
            search.create({
                type: 'salesorder',
                filters: [
                    { name: 'custbody_order_type', operator: 'is', values: 5},
                    { name: 'custbody_dps_waybill_no', operator: 'isempty', values: [] },
                    { name: 'custbody_dps_fulfillment_order_id', operator: 'isnotempty', values: [] },
    ...
    function GetFulfillmentOrder(acc_id, SellerFulfillmentOrderId,delivery_order_id){
        ...
        if (auth) {
            ...
            var res = xml.Parser.fromString({ text: response.replace('xmlns','aaa') });
