 ...
 *@NScriptType Restlet
 */
define(['N/record', 'N/log', 'N/search', '../../Helper/md5', '../../Helper/CryptoJS.min', 'N/encode', 'N/https', 'N/xml', 'N/format'],
function (record, log, search, md5, cryptoJS, encode, https, xml, format) {
        ...
        try {
            ...
            if (action == 'CreateFulfillmentOrder') {
                ...
                search.create({
                    ...
                    columns: [ 'item', 'custbody_sotck_account', 'custbody_dps_order_contact', 'custcol_aio_amazon_msku', 'quantity',
                        'trandate', 'tranid', 'custbody_mcf_displayable_order_comment', 'custbody_mcf_fulfillment_action', 'custbody_mcf_delivery_method',
                        'custbodyfulfillment_stock_so', 'custbody_reason_type', // 原因类型
                        { name: 'custentity_store_number', join: 'customer' }, // 12 店铺编号
                        'otherrefnum' // 13 原订单号
                    ]
            ...
            if (action == 'CancelFulfillmentOrder') {
                var fulPo, delivery_shop, fulInfo;
                search.create({
                    type: 'salesorder',
                    filters: [
                        { name: 'internalid', operator: 'is', values: context.recId },
            ...
            if (action == 'UpdateSOTypeToMCF') {
                var so_id = context.recId;
                var so_trandate;
                search.create({
                    type: record.Type.SALES_ORDER,
                    filters: [{ name: 'internalid', operator: 'is', values: so_id }],
                    columns: [ 'trandate' ]
                }).run().each(function (result) {
                    so_trandate = result.getValue('trandate');
                    return false;
                });
                var so_copy_record = record.copy({
                    type: record.Type.SALES_ORDER,
                    id: so_id
                });
                so_copy_record.setValue({ fieldId: 'trandate', value: format.parse({ type: format.Type.DATE, value: so_trandate }) });
                var count = so_copy_record.getLineCount({ sublistId: 'item' });
                for (var i = 0; i < count; i++) {
                    so_copy_record.setSublistValue({ sublistId: 'item', fieldId: 'isclosed', line: i, value: true });
                }
                var so_copy_id = so_copy_record.save();
                if (so_copy_id) {
                    record.submitFields({
                        type: record.Type.SALES_ORDER,
                        id: so_id,
                        values: {
                            ...
                            custbody_order_type_fulfillment: 3
                        },
                        options: {
                            enableSourcing: false,
                            ignoreMandatoryFields: true
                ...
                result.code = 'Success';
                result.message = '变更成功，原始订单已保存并关闭';
            }
        } catch (error) {
            log.error('出错了', error);
            result.code = '失败';
    ...
    var core1 = {
        getAuthByAccountId: function (account_id) {
            ...
            search.create({
                ...
                columns: [
                    'custrecord_aio_seller_id', 'custrecord_aio_mws_auth_token',
