 ...
 * @LastEditTime   : 2020-05-16 14:23:39
 * @LastEditors    : Li
 * @Description    : 
 ...
 */
define(['../Helper/Moment.min', 'N/search', 'N/format', 'N/record', 'N/runtime', 'N/redirect'],
    function (moment, search, format, record, runtime, redirect) {

        function beforeLoad(context) {
            var type = context.type;
            var userObj = runtime.getCurrentUser();
            var userName = userObj.name;
            if (type == context.UserEventType.EDIT) { //如果是已通过的编辑状态下
                ...
                if (request != null) {
                    ...
                    var checkStatus = newRecord.getText('custrecord_vmph_check_status');
                    var creator = newRecord.getText('custrecord_vendor_price_creator');
                    if (checkStatus == '已通过' || checkStatus == '审批中') {
                        // redirect.redirect({
                        //     url: reqUrl + '?rectype=' + rectype + '&id=' + recId
                        // });
                    ...
                    if (checkStatus == '待提交审批' && userName != creator) { //如果是待提交审批且当前用户不等于创建人，则不能编辑
                        // redirect.redirect({
                        //     url: reqUrl + '?rectype=' + rectype + '&id=' + recId
                        // });
