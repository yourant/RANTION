 ...
 * @LastEditTime   : 2020-07-16 16:05:26
 * @LastEditors    : Li
 * @Description    : 
 * @FilePath       : \Rantion\wms\rantion_wms_create_inmaster_re_rl copy 2.js
...
define(['../Helper/config.js', 'N/search', 'N/record', 'N/log', '../common/request_record',
    ...
    function returnTransfer(context) {
        ...
        if (to_id) { // 存在对应的第二段调拨单
            ...
            var toItemObj = tool.searchTransactionItemObj(recType, to_id);
            BoxObjKey.map(function (boxKey) {
                ...
                for (var j = 0, jLen = temp_obj.length; j < jLen; j++) {
                    ...
                    if (toItemObj[dl_sku.sku]) { // 若存在对应的货品

                        var lineNumber = irObj.findSublistLineWithValue({
                            sublistId: 'item',
                            fieldId: 'item',
                            value: toItemObj[dl_sku.sku]
                        }); // 获取对应货品所在的行号
