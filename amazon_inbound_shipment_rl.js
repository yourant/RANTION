 ...
 * @LastEditTime   : 2020-07-13 11:36:10
 */
/**
 *@NApiVersion 2.x
 *@NScriptType Restlet
 */
define(['./CryptoJS.min', '../Helper/config', 'N/search', 'N/record', 'N/xml', 'N/https', 'N/crypto', 'N/file', 'N/encode', 'N/runtime'], 
    ...
    var core = {
        ...
        getAddressById: function (address_id) {
            ...
            search.create({
                ...
                filters: [
                    { name: 'internalid', operator: 'is', values: address_id }
                ], 
                columns: [
                    { name: 'name' },
        ...
        getFeedSubmissionList: function (account_id, submission_ids, recId , recType) {
            ...
            var res = xml.Parser.fromString({ text: content });
            var feeds = [];
            res.getElementsByTagName({ tagName: 'FeedSubmissionInfo' }).map(function (info) {
                feeds.push({
            ...
            feeds.map(function (feed) {
                search.create({
                    ...
                    filters: [
                        { name: 'custrecord_aio_feed_account', operator: search.Operator.ANYOF, values: account_id },
                        { name: 'custrecord_aio_feed_submission_id', operator: search.Operator.IS, values: feed.feed_submission_id },
                        { name: 'custrecord_aio_feed_bg', operator: search.Operator.ANYOF, values: recId }
                    ],
                    columns: [
                    ...
                    if (feed.feed_processing_status == '_DONE_' && "" + rec.getValue(rec.columns[0]) == '') {
                        response = core.mwsRequestMaker(auth, 'GetFeedSubmissionResult', '2009-01-01', { FeedSubmissionId: feed.feed_submission_id }, '/Feeds/2009-01-01/');
                    }
                    record.submitFields({
                        type: 'customrecord_aio_amazon_feed',
                        id: rec.id,
        ...
        submitCartonContent: function (account_id, recId, recType, shipment_id) {
            var auth = core.getAuthByAccountId(account_id), bg_rec = record.load({ type: recType, id: recId, isDynamic: true });
            
            var result_data = {};
            if (auth) {
                var params = {}, carton_infos_1 = {};
                search.create({
                    type: 'customrecord_bulky_boxing_info', 
                    filters: [
                        { name: 'custrecord_bulky_shipment_id', join:'custrecord_bulky_shipping_id', operator: 'is', values: shipment_id }
                    ], 
                    columns: [
                ...
                if (res.getElementsByTagName({ tagName: 'FeedProcessingStatus' }).length > 0) {
                    var feed = record.create({ type: 'customrecord_aio_amazon_feed', isDynamic: true });
                    feed.setValue({ fieldId: 'custrecord_aio_feed_account', value: account_id });
                    feed.setValue({ fieldId: 'custrecord_aio_feed_submission_id', value: core.getTextContentSafe(res, 'FeedSubmissionId') });
                    feed.setValue({ fieldId: 'custrecord_aio_feed_type', value: core.getTextContentSafe(res, 'FeedType') });
                    feed.setValue({ fieldId: 'custrecord_aio_submitted_date', value: core.getTextContentSafe(res, 'SubmittedDate') });
                    feed.setValue({ fieldId: 'custrecord_aio_feed_processing_status', value: core.getTextContentSafe(res, 'FeedProcessingStatus') });
                    feed.setValue({ fieldId: 'custrecord_aio_started_processing_date', value: core.getTextContentSafe(res, 'StartedProcessingDate') });
                    feed.setValue({ fieldId: 'custrecord_aio_completed_processing_date', value: core.getTextContentSafe(res, 'CompletedProcessingDate') });
                    feed.setValue({ fieldId: 'custrecord_aio_feed_body', value: body_1 });
                    feed.setValue({ fieldId: 'custrecord_aio_feed_bg', value: recId });
                    feed.save();

                    bg_rec.setValue({ fieldId: 'custrecord_bg_feed_submission_id', value: core.getTextContentSafe(res, 'FeedSubmissionId') });
                    bg_rec.setValue({ fieldId: 'custrecord_bg_feed_type', value: core.getTextContentSafe(res, 'FeedType') });
                    bg_rec.setValue({ fieldId: 'custrecord_bg_submitted_date', value: core.getTextContentSafe(res, 'SubmittedDate') });
                    bg_rec.setValue({ fieldId: 'custrecord_bg_feed_processing_status', value: core.getTextContentSafe(res, 'FeedProcessingStatus') });
                    bg_rec.setValue({ fieldId: 'custrecord_bg_started_processing_date', value: core.getTextContentSafe(res, 'StartedProcessingDate') });
                    bg_rec.setValue({ fieldId: 'custrecord_bg_completed_processing_date', value: core.getTextContentSafe(res, 'CompletedProcessingDate') });
                    bg_rec.setValue({ fieldId: 'custrecord_bg_feed_body', value: body_1 });

                    result_data.status = 'success';
                    result_data.message = res.getElementsByTagName({ tagName: 'FeedProcessingStatus' })[0].textContent;
                    return result_data
        ...
        getPackageLabels: function (account_id, shipment_id, recId, recType) {
            ...
            if (auth) {
                ...
                search.create({
                    type: 'customrecord_bulky_boxing_info', 
                    filters: [
                        { name: 'custrecord_bulky_shipping_id', operator: 'anyof', values: recId }
                    ],
                    columns: [
                        { name: 'custrecord_bulky_box_case_number', summary: 'group' }
                    ]
                }).run().each(function (rec) {
                    key_2++;
                    return true;
                ...
                var res = xml.Parser.fromString({ text: content });
                log.debug('res',res);
                if (res.getElementsByTagName({ tagName: 'PdfDocument' }).length > 0) {
                    result_data.status = 'success';
                    result_data.message = res.getElementsByTagName({ tagName: 'PdfDocument' })[0].textContent;
                    return result_data
                    return res.getElementsByTagName({ tagName: 'PdfDocument' })[0].textContent;
                }
                else {
                    result_data.status = 'error';
                    result_data.message = "\u4ECE\u4E9A\u9A6C\u900A\u83B7\u53D6\u4FE1\u606F\u5931\u8D25\uFF0C\u8BE6\u7EC6\u8FD4\u56DE\u4FE1\u606F: " + content;
        ...
        updateInboundShipment: function (account_id, recId, recType, shipment_id, shipment_status, order_contact) {
            var auth = core.getAuthByAccountId(account_id), bg_rec = record.load({ type: recType, id: recId, isDynamic: true }), addr = core.getAddressById(Number(order_contact));
            var result_data={};
            ...
            if (auth && addr) {
                ...
                var line_count = bg_rec.getLineCount({ sublistId: 'recmachcustrecord_bulky_relative_shipping_rec' });
                for (var key = 0; key < line_count; key++) {
                    params["InboundShipmentItems.member." + (key + 1) + ".QuantityShipped"] = bg_rec.getSublistValue({ sublistId: 'recmachcustrecord_bulky_relative_shipping_rec', fieldId: 'custrecord_bulky_quantity', line: key });
                    params["InboundShipmentItems.member." + (key + 1) + ".SellerSKU"] = bg_rec.getSublistValue({ sublistId: 'recmachcustrecord_bulky_relative_shipping_rec', fieldId: 'custrecord_bulky_platform_sku', line: key });
                }
                ...
                var rtn_shipment_id = xml.XPath.select({ node: res, xpath: "//nlapi:UpdateInboundShipmentResult/nlapi:ShipmentId" }).map(function (_) { return _.textContent; }).join('');
                if (rtn_shipment_id) {
                    bg_rec.setText({ fieldId: 'custrecord_bg_inbound_shipment_status', text: core.listInboundShipments(auth, null, [rtn_shipment_id]).shift().shipment_status });
                    bg_rec.save();
                    result_data.status = 'success';
        ...
        listInboundShipments: function(auth, nextToken, shipmentIds) {
            ...
            if (nextToken) {
                ...
                var params_2 = {};
                shipmentIds.map(function(id, key) { return params_2["ShipmentIdList.member." + (key + 1)] = id; });
            ...
            var res = xml.Parser.fromString({ text: content });
            res.getElementsByTagName({ tagName: 'member' }).map(function(node) {
                shipments.push({
                    ...
                    ship_from_address: res.getElementsByTagName({ tagName: 'ShipFromAddress' }).length ? {
                        id: '',
                        Name: core.getTextContentSafe(res.getElementsByTagName({ tagName: 'ShipFromAddress' })[0], 'Name'),
                        AddressLine1: core.getTextContentSafe(res.getElementsByTagName({ tagName: 'ShipFromAddress' })[0], 'AddressLine1'),
                        AddressLine2: core.getTextContentSafe(res.getElementsByTagName({ tagName: 'ShipFromAddress' })[0], 'AddressLine2'),
                        City: core.getTextContentSafe(res.getElementsByTagName({ tagName: 'ShipFromAddress' })[0], 'City'),
                        DistrictOrCounty: core.getTextContentSafe(res.getElementsByTagName({ tagName: 'ShipFromAddress' })[0], 'DistrictOrCounty'),
                        StateOrProvinceCode: core.getTextContentSafe(res.getElementsByTagName({ tagName: 'ShipFromAddress' })[0], 'StateOrProvinceCode'),
                        CountryCode: core.getTextContentSafe(res.getElementsByTagName({ tagName: 'ShipFromAddress' })[0], 'CountryCode'),
                        PostalCode: core.getTextContentSafe(res.getElementsByTagName({ tagName: 'ShipFromAddress' })[0], 'PostalCode'),
                    } : null,
                    items: []
                });
            });
            if (res.getElementsByTagName({ tagName: 'NextToken' }).length > 0) {
                core.listInboundShipments(auth, res.getElementsByTagName({ tagName: 'NextToken' })[0].textContent).map(function(shipment) { return shipments.push(shipment); });
            }
            return shipments;
        },
