...
define(["N/format", "N/runtime", "./Helper/core.min", "./Helper/CryptoJS.min",
    "./Helper/Moment.min", "N/log", "N/search", "N/record", "N/transaction", "N/encode", "N/https", "N/xml", 'N/config'
        ...
        try {
            ...
            core.amazon.getAccountList().map(function (account) {
                ...
                if (account.enabled_sites == 'AmazonUS') {
                    ...
                    last_updated_after = "2020-03-05T08:00:00.000Z";
                    ...
                    last_updated_after = "2020-03-05T09:00:00.000Z";
                ...
                if (request_date) {
                    ...
                    // 设置统一时间
                    last_updated_after = "2020-04-01T00:00:00.000Z";
                    last_updated_before = "2020-04-09T17:00:00.000Z";
    ...
    function map(context) {
        ...
        log.debug("order:", JSON.stringify(context.value))
        // ====================进cache==============
        try {
            var r;
            search.create({
            ...
            var order_trandate = moment.utc(order.purchase_date).toDate();
            var last_update_date = moment.utc(order.last_update_date).toDate();
            r.setValue({
            ...
            r.setValue({
                ...
                value: order_trandate
            });
            r.setValue({
                fieldId: 'custrecord_trandate_datetime',
                value: order_trandate
            });
            r.setValue({
        ...
        handleit: function (acc_id, last_after, last_before) {
            ...
            if (!last_update_date) {
                ...
                if (enabled_sites == 'AmazonUS') {
                    ...
                    last_update_date = "2020-04-01T00:00:00.000Z";
            ...
            if (hid && nextToken) {
                ...
                h.setValue({
                    fieldId: 'custrecord_aio_importer_last_updated_af',
                    //     value: last_update_date
                    value: moment(last_update_date).toDate()
                });
