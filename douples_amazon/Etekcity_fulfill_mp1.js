 ...
 * @LastEditors    : Li
 * @LastEditTime   : 2020-04-21 18:07:03
 * @Description    : 
 * @FilePath       : \douples\Etekcity_fulfill_mp.js
 * @Author         : Li
...
define(["N/runtime", 'N/search', 'N/record', './Helper/Moment.min.js'],
    function (runtime, search, record, moment) {
        ...
        function getInputData() {
            var limit = 3, orders = [], amazon_location_no;
            var acc = runtime.getCurrentScript().getParameter({ name: 'custscript_if_account' });
            var fils = []
            fils.push(search.createFilter({ name: 'custrecord_fulfill_in_ns', operator: search.Operator.IS, values: false }));
            ...
            try {
                ...
                search.create({
                    ...
                    filters: [
                        ...
                        { name: "status", operator: "anyof", values: ["SalesOrd:A", "SalesOrd:B", "SalesOrd:E", "SalesOrd:D"] },
        ...
        function fullfillment(so_id, shipdate, ship_sku, qty, rei, amazon_location_no) {
            ...
            search.create({
                ...
                filters: [
                    { name: 'custrecord_dps_amazon_location_number', operator: 'is', values: amazon_location_no }
                ],
                columns: [
            ...
            for (var ii = 0; ii < numLines; ii++) {
                // 获取货品
                var so_item_id = so.getSublistValue({ sublistId: 'item', fieldId: 'item', line: ii });
                // 获取货品数量
                var so_item_quantity = so.getSublistValue({ sublistId: 'item', fieldId: 'quantity', line: ii });
                var it = {
            ...
            if (ord_sta == 'B' || ord_sta == 'E' || ord_sta == 'D') {
                ...
                f.setValue({ fieldId: 'trandate', value: moment.utc(shipdate).toDate() });
                f.setValue({ fieldId: 'shipstatus', value: 'C' });
                f.setValue({ fieldId: 'custbody_pr_store', value: so.getValue('custbody_pr_store') });
                ...
                for (var ln = 0; ln < lc; ln++) {
                    var po_item_id = f.getSublistValue({ sublistId: 'item', fieldId: 'item', line: ln });
                    ...
                    for (var jj = 0; jj < item_arr.length; jj++) {
                        if (item_arr[jj].so_item_id == po_item_id) {
                            f.setSublistValue({ sublistId: 'item', fieldId: 'quantity', value: item_arr[jj].so_item_quantity, line: ln });
                        }
                    }
            ...
            try {
                ...
                search.create({
                    ...
                    filters: [
                        ...
                        { name: 'custrecord_seller_sku_code', operator: 'is', values: sku.trim() },//sku
                        { name: 'custrecord_seller_sku_account', operator: 'is', values: amazon_account_id }//店铺ID
                    ],
                    columns: [
                        'custrecord_item_sku'
                    ]
                ...
                if (skuid.length < 1) {
                    search.create({
                        type: 'item',
                        filters: [
                            { name: 'itemid', operator: 'is', values: sku.trim() }
                        ]
                    }).run().each(function (rec) {
                        skuid.push(rec.id)
                        return false;
        ...
        function Inventory_Nunmber(f, l, quantity, location, type, po, subsidiary) {
            var item = f.getSublistValue({ sublistId: 'item', fieldId: 'item', line: l }), qty = quantity
                ...
                for (var i = 0; i < invennumber.length; i++) {
                    ...
                    if (qty >= 0) {
                        if (type == 'so') {
                            ...
                            inventorydetail.setSublistValue({ sublistId: 'inventoryassignment', fieldId: 'receiptinventorynumber', value: invennumber[i].inventorynumber, line: line_count });
                        } else {
                            log.debug("赋值批次号", invennumber[i].inventorynumber)
                            inventorydetail.setSublistText({ sublistId: 'inventoryassignment', fieldId: 'issueinventorynumber', Text: invennumber[i].inventorynumber, line: line_count });
                            inventorydetail.setSublistValue({ sublistId: 'inventoryassignment', fieldId: 'receiptinventorynumber', value: invennumber[i].inventorynumber, line: line_count });
                        }
                        inventorydetail.setSublistValue({ sublistId: 'inventoryassignment', fieldId: 'quantity', value: line_count == 0 ? quantity : Number(type == 'so' ? invennumber[i].quantityavailable : invennumber[i].quantityintransit) - Number(qty), line: line_count });
                        line_count++;
                        break;
                    } else {
                        qty = 0 - Number(qty)
                        ...
                        inventorydetail.setSublistValue({ sublistId: 'inventoryassignment', fieldId: 'quantity', value: type == 'so' ? invennumber[i].quantityavailable : invennumber[i].quantityintransit, line: line_count });
                        line_count++;
                    }
                }
            }
        ...
        function createInvioce(soid, shipdate, acc) {
            ...
            try {
                ...
                inv.setValue({ fieldId: 'custbody_pr_store', value: acc });
                // inv.setValue({ fieldId: 'approvalstatus', value: 2 });
                inv.setValue({ fieldId: 'trandate', value: moment.utc(shipdate).toDate() });
