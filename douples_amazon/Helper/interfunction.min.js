...
define(['N/record', 'N/search', './core.min', 'N/format', './Moment.min', 'N/runtime',
     ...
     * @param {*} acc_search
     */
    function SearchSO(order_id, merchant_order_id, acc_search, cl_date, Trp, isSorder) {
        var fils = [
                { name: 'mainline', operator: 'is', values: true }
            ],
            rs = {}
        var SS = false
        log.debug('isSorder:', isSorder);
        log.debug('order_id:', order_id);
        if (order_id.charAt(0) == 'S' && !isSorder) {
            log.debug('是属于S开头的订单', 'amazon orderid : ' + order_id + ',seller orderid: ' + merchant_order_id)
            SS = true
            // fils.push({ name: 'custbody_sotck_account', operator: 'anyof', values: acc_search }) // 发货店铺
            // if (merchant_order_id) {
            //     fils.push({ name: 'poastext', operator: 'is', values: order_id })
            // }


            var soid = searchMappSO(order_id, merchant_order_id); // 搜索映射关系表, 获取对应的
            log.audit("找到的订单号", soid);

            if (!soid && merchant_order_id) {
                // 直接搜索对应的原订单, 进行发货开票
                // if (merchant_order_id) {
                fils.push({ name: 'custbodyfulfillment_stock_so', operator: 'is', values: merchant_order_id })
            } else if (soid) {
                fils.push({ name: 'poastext', operator: 'is', values: soid })
            } else {
                fils.push({ name: 'poastext', operator: 'is', values: order_id })
            }
            ...
            log.audit('订单号不存在,return fasle')
            return false
        }
        log.debug('order fils:', fils);
        search.create({
            ...
            columns: [
                ...
                { name: 'custrecord_aio_fbaorder_location', join: 'custbody_sotck_account' },
                { name: 'entity' },
                { name: 'subsidiary' },
                { name: 'currency' },
            ...
            if (rec.getValue('status') == 'closed' || rec.getValue('status') == 'pendingApproval') {
                return true;
            }
            rs['so_id'] = rec.id
            ...
            rs['acc_loca'] = rec.getValue(rec.columns[2])
            rs['ord_loca'] = rec.getValue('location')
            rs['ord_type'] = rec.getValue('custbody_order_type')
            if (SS)
            ...
            if (rec.getValue('status') == 'pendingFulfillment' && rec.getValue('custbody_aio_s_fulfillment_channel') == 'MFN') {
                ...
                if (Trp == 'order') {
                    ...
                    if (cl_date) {
                        ft.push({ name: 'custrecord_shipment_date', operator: search.Operator.ONORBEFORE, values: cl_date });
                    }
                    search.create({
    ...
    function getskuId(sellersku, amazon_account_id, amazon_order_id) {
        ...
        search.create({
            ...
            columns: [
                { name: 'custrecord_ass_sku' },
                { name: 'locationaveragecost', join: 'custrecord_ass_sku' }
            ]
        }).run().each(function(rec) {
            skuid = rec.getValue('custrecord_ass_sku')
            averagecost = rec.getValue(rec.columns[1])
            return false
        })
        if (!skuid) {
            ...
            log.debug('找不到货品, 或者货品已经非活动了(SKU): ' + sellersku.trim())
            return false
        }
        var rs = { 'skuid': skuid, 'averagecost': averagecost }
        return rs
    }
    ...
    function getSearchAccount(sellerid) {
        ...
        search.create({
            ...
            if (rec.getValue('custrecord_dps_get_report')) {
                key = rec.id
            }
    ...
    function GetstoreInEU(acc, marketplace_name, acc_text) {
        ...
        if (marketplace_name.toUpperCase() == 'OM') {
            marketplace_name = 'us'
        }
        search.create({
            type: 'customrecord_aio_account',
     ...
     * @param {*} memomain
     */
    function CheckJO(orderid, settlmentid, settlement_ids, memomain, acc, merchant_order_id) {
        log.debug('memomain:' + memomain, 'settlement_ids：' + settlement_ids)
        log.debug('search 的 settlmentid:' + settlmentid, acc)
        var getc, fils = [{ name: 'mainline', operator: 'is', values: true },
            { name: 'custbody_amazon_settlementid', operator: 'is', values: settlmentid }
        ]
        if (orderid) {
            fils.push({ name: 'custbody_jour_orderid', operator: 'is', values: orderid })
        } else {
            fils.push({ name: 'custbody_jour_orderid', operator: 'isempty' })
        }
        if (!orderid && merchant_order_id) {
            fils.push({ name: 'custbody_setl_merchant_order_id', operator: 'is', values: merchant_order_id })
        }
        if (memomain)
            fils.push({ name: 'memomain', operator: 'is', values: memomain })
        else {
            fils.push({ name: 'memomain', operator: 'isempty' })
        }

        log.audit("CheckJO fils  过滤器", fils)
        search.create({
            type: 'journalentry',
            filters: fils
        }).run().each(function(e) {
     ...
     */
    function relativaJE(jo1_ids, jo2_id, jo3_id, settlement_ids, memo_obj, setlle_acc, jourType) {
        ...
        if (memo_obj.length < 10 && memo_obj) {
            memo_obj.map(function(inv) {
                var insv = record.load({ type: 'invoice', id: inv.inv_id })
                insv.setValue({ fieldId: 'custbody_setllement_depositamount', value: inv.depAmaount })
                insv.setText({ fieldId: 'custbody_dp_expected_due_date', text: inv.end_date })
                insv.save({
                    ignoreMandatoryFields: true
                })
                // record.submitFields({
                ...
                //   id: inv.inv_id,
                //   values: {
                //     custbody_setllement_depositamount: inv.depAmaount,
                //     custbody_dp_expected_due_date: inv.end_date
                //   }
                // })
            })
        }
        if (jo2_id && jo1_ids.length < 10) {
            record.submitFields({
                type: 'journalentry',
                id: jo2_id,
                values: {
        ...
        try {
            setTruesettlment(settlement_ids, setlle_acc, jourType)
        } catch (e) {
            record.submitFields({
                type: 'journalentry',
                id: jo2_id,
    ...
    function setTruesettlment(settlement_ids, setlle_acc, jourType) {
        log.debug('接口脚本 settlement_ids :', settlement_ids)
        for (var index = 0; index < settlement_ids.length; index++) {

            var valu = {};
            if (setlle_acc) {
                valu.custrecord_settlement_acc = setlle_acc;
            }
            valu.custrecord_february_undeal = "";

            if (jourType == 2) { // 标记已生成冲销
                valu.custrecord_settle_is_write_voucher = true
            } else { // 标记已生成结算
                valu.custrecord_settle_is_generate_voucher = true
            }
            record.submitFields({
                type: 'customrecord_aio_amazon_settlement',
     ...
     */
    function GetSettlmentFee(Amount_type, Amount_desc, Tranction_type, markectplace, amount, siteId, pr_store, orderid) {
        // log.debug('c审查费用类型', {
        //   'Tranction_type': Tranction_type,'Amount_type': Amount_type,'Amount_desc': Amount_desc,
        //   'markectplace': markectplace,'siteId': siteId,'pr_store': pr_store,'orderid': orderid
        // })
        var fils = []
        ...
        if (incomeaccount) {
            ...
            unfee_rec.setValue({
                fieldId: 'custrecord_settlement_report_rel',
                value: orderid
            })
            ...
            log.debug('找不到对应的费用类型，已记录:' + ss + ',pr_store:' + pr_store, Tranction_type + ',' + Amount_type + ',' + Amount_desc)
            return false
    ...
    /**
     * 检查是否是 应收费用，不计入费用 ; 或者是不用处理的费用
     */
    function getArFee(Tranction_type, Amount_type, Amount_desc, currency_txt) {
        if (
            (((Tranction_type == 'Order' && Amount_type == 'ItemPrice' && Amount_desc == 'GiftWrap') ||
                    (Tranction_type == 'Order' && Amount_type == 'ItemPrice' && Amount_desc == 'Principal') ||
                    (Tranction_type == 'Order' && Amount_type == 'ItemPrice' && Amount_desc == 'Shipping') ||
                    (Tranction_type == 'Order' && Amount_type == 'Promotion' && Amount_desc == 'Principal') ||
                    (Tranction_type == 'Order' && Amount_type == 'Promotion' && Amount_desc == 'Shipping') ||
                    ...
                    (Tranction_type == 'other-transaction' && Amount_type == 'other-transaction' && Amount_desc == 'Transfer of funds unsuccessful: An unexpected issue occurred. Please contact Seller Support for more information.') ||
                    (Tranction_type == 'other-transaction' && Amount_type == 'other-transaction' && Amount_desc == 'Previous Reserve Amount Balance') ||
                    (Tranction_type == 'other-transaction' && Amount_type == 'other-transaction' && Amount_desc == 'Transfer of funds unsuccessful: A transfer could not be initiated because the transfer amount does not meet Amazon’s minimum transfer amount.') ||
                    (Tranction_type == 'other-transaction' && Amount_type == 'other-transaction' && Amount_desc == 'Current Reserve Amount') ||
                    (Tranction_type == 'other-transaction' && Amount_type == 'other-transaction' && Amount_desc == 'Transfer of funds unsuccessful: Amazon has cancelled your transfer of funds.') ||
                    (Tranction_type == 'other-transaction' && Amount_type == 'other-transaction' && Amount_desc == 'Payable to Amazon')) &&
                (currency_txt == 'USD' || currency_txt == 'CAD' || currency_txt == 'MXD' || currency_txt == 'AUD' || currency_txt == 'SGD')) ||
            (((Tranction_type == 'Order' && Amount_type == 'ItemPrice' && Amount_desc == 'Principal') ||
                    (Tranction_type == 'Order' && Amount_type == 'ItemPrice' && Amount_desc == 'Shipping') ||
                    (Tranction_type == 'Order' && Amount_type == 'ItemPrice' && Amount_desc == 'ShippingTax') ||
                    (Tranction_type == 'Order' && Amount_type == 'ItemPrice' && Amount_desc == 'Tax') ||
                    (Tranction_type == 'Order' && Amount_type == 'ItemPrice' && Amount_desc == 'GiftWrapTax') ||
                    (Tranction_type == 'Order' && Amount_type == 'ItemPrice' && Amount_desc == 'GiftWrap') ||
                    (Tranction_type == 'Order' && Amount_type == 'Promotion' && Amount_desc == 'Shipping') ||
                    (Tranction_type == 'Order' && Amount_type == 'Promotion' && Amount_desc == 'TaxDiscount') ||
                    (Tranction_type == 'Order' && Amount_type == 'Promotion' && Amount_desc == 'Principal') ||
                    (Tranction_type == 'Chargeback Refund' && Amount_type == 'ItemPrice' && Amount_desc == 'Principal') ||
                    (Tranction_type == 'Chargeback Refund' && Amount_type == 'ItemPrice' && Amount_desc == 'Shipping') ||
                    ...
                    (Tranction_type == 'other-transaction' && Amount_type == 'other-transaction' && Amount_desc == 'Payable to Amazon') ||
                    (Tranction_type == 'other-transaction' && Amount_type == 'other-transaction' && Amount_desc == 'Transfer of funds unsuccessful: A transfer could not be initiated because the transfer amount does not meet Amazon’s minimum transfer amount.') ||
                    (Tranction_type == 'other-transaction' && Amount_type == 'other-transaction' && Amount_desc == 'Transfer of funds unsuccessful: We could not transfer funds to your bank account because the account information on file is invalid. Please update your bank account')) &&
    ...
    function getFormatedDate(postDate, endDate, depositDate, startDate, is_tran) {
        ...
        if (!depositDate && postDate && endDate && startDate) {
     ...
     */
    function li_dateFormat(date, fmt) {
        var o = {
            "M+": date.getMonth() + 1,
            "d+": date.getDate(),
            "h+": date.getHours(),
    ...
    /**
     *  通过映射关系获取 S 单号对应的 原订单号
     * @param {String} orderid
     * @param {Strign} m_orderid
     */
    function searchMappSO(orderid, m_orderid) {
        ...
        if (orderid) {
            search.create({
                ...
                filters: [{ name: 'custrecord_amazon_order_s', operator: 'is', values: orderid }],
                columns: ["name"]
            }).run().each(function(rec) {
                origin_order = rec.getValue("name");
            });
        }
