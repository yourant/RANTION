 ...
 * @LastEditTime   : 2020-09-30 15:34:51
 * @LastEditors    : Li
...
define(['N/search', 'N/record', './Helper/Moment.min', 'N/format', 'N/runtime',
    ...
    function getInputData() {
        ...
        var group = runtime.getCurrentScript().getParameter({ name: 'custscript_dps_li_sett_jour_w_acc_group' }); // 店铺组
        var runPaged = runtime.getCurrentScript().getParameter({ name: 'custscript_dps_li_sett_jour_w_runpage' }); // 运行方式
        var par_orid = runtime.getCurrentScript().getParameter({ name: 'custscript_dps_li_sett_jour_w_order_id' }); // 订单号
        log.audit('选择的店铺:' + acc, 'par_orid:' + par_orid)

        var _start_date = runtime.getCurrentScript().getParameter({ name: 'custscript_dps_li_sett_jour_w_start_date' }); // 开始时间
        var _end_date = runtime.getCurrentScript().getParameter({ name: 'custscript_dps_li_sett_jour_w_end_date' }); // 结束时间
        ...
        try {
            ...
            search.create({
                ...
                columns: [
                    ...
                    { name: 'custrecord_aio_sett_sku' }, // msku
                ]
                ...
                if (!settlement_acc) {
                    var markt
                    if (JSON.stringify(rec.getValue('custrecord_aio_sett_marketplace_name')).indexOf('Amazon.') == -1) {
                        search.create({
                            type: 'customrecord_aio_amazon_settlement',
                            filters: [
                ...
                if (!settlement_acc) {
                    ...
                    temp_msku = rec.getValue('custrecord_aio_sett_sku');
                    if (JSON.stringify(mskuArr).indexOf(temp_msku) == -1) {
                        mskuArr.push(temp_msku) // msku 用于 S订单
                    }
                    ...
                    if (ck || (!Tranction_type)) {
                        ...
                        if (!mok) {
                            ...
                            settlement_ids = [];
                            // if (settlement_idObj[settle_id + '-' + month]) {
                            //     settlement_ids = settlement_idObj[settle_id + '-' + month]
                            // }
            ...
            manual.map(function(mnu) {
                record.submitFields({
                    type: 'customrecord_aio_amazon_settlement',
                    id: mnu,
                    values: { custrecord_is_manual: true }
                })
            })
            ...
            search.create({
                ...
                columns: [{ name: 'custrecord_aio_seller_id' },
            ...
            if (!so_id) {
                ...
                if (orderid.indexOf("S") > -1) {
                    orderid = orderid;
                    pr_store = report_acc;
                    search.create({
                        type: "customrecord_aio_account",
                        filters: [{ name: "internalid", operator: "anyof", values: report_acc }],
                        columns: ["custrecord_aio_customer"]
                    }).run().each(function(rec) {
                        entity = rec.getValue("custrecord_aio_customer");
                    });
                } else {
                ...
                if (interfun.CheckJO(orderid, key, settlement_idObj[key], '冲销', search_accObj.acc_search, merchant_order_id)) {
                    context.write({
                        ...
                        value: {
                            ...
                            'report_siteId': report_siteId,
                            'item_codes': item_codes, // item code
    ...
    function reduce(context) {
        ...
        v.map(function(obj) {
            ...
            try {
                ...
                // UPDATE  2020-09-29  11:11   去掉  S 单转换逻辑
                /*
                ...
                */
                ...
                jour.setValue({ fieldId: 'memo', value: '冲销' })
                ...
                jour.setValue({ fieldId: 'custbody_curr_voucher', value: '冲销凭证' })
                ...
                var rs
                if (orderid && CK_fin) {
                    rs = GetInfo_JO1(orderid, search_acc, item_codes, order_id, mskuArr)

                    log.audit("查找对应的预估凭证", rs);
                    if (!rs) {
                        ...
                        search.create({
                            ...
                            filters: [
                                ...
                                { name: 'custrecord_shipment_account', operator: 'anyof', values: search_acc }
                            ]
                        }).run().each(function(e) {
                            shipId = e.id
                        })
                        ...
                        if (shipId) {
                            ...
                            search.create({
                                ...
                                filters: [
                                    { name: 'custrecord_fin_to_amazon_account', operator: 'anyof', values: mainAcc },
                                    { name: 'custrecord_l_amazon_order_id', operator: 'is', values: order_id },
                ...
                interfun.relativaJE(jo1_id, jo_2, '', settlement_ids, orders, pr_store, 2)
    ...
    function GetInfo_JO1(orderid, search_acc, item_codes, order_id, seller_sku) {
        var jo1_arrys = [],
            relative_finance = [],
            jo1_id = []
        var fils_fee = [
        ...
        if (orderid.indexOf("S") > -1) {
            ...
            seller_sku.map(function(msku) {
                if (msku) {
                    fil.push(['custbody_relative_finanace_report.custrecord_sellersku', 'is', msku])
                    fil2.push(['custrecord_sellersku', 'is', msku]);
                } else {
                    fil.push(['custbody_relative_finanace_report.custrecord_sellersku', 'isempty', ''])
                    fil2.push(['custrecord_sellersku', 'isempty', '']);
                }
        ...
        log.audit('000000item code fin_fils', fils_fee)
        search.create({
            type: 'journalentry',
            filters: fils_fee,
            columns: [
