...
define(['N/search', 'N/record', './Helper/Moment.min', 'N/format', 'N/runtime', "./Helper/interfunction.min", "./Helper/core.min", ],
    function (search, record, moment, format, runtime, interfunction, core) {
        ...
        const acc_sub_corr = {
            ...
            "34": {
                "subsidiary": 85,
                "entity": 811596
            }, //  EtekcityCorporationEU   ETEKCITY Corporation (US)
            "24": {
                "subsidiary": 85,
                "entity": 811595
            }, // EtekcityCorporationES       ETEKCITY Corporation (US)
            "22": {
                "subsidiary": 85,
                "entity": 811597
            }, // EtekcityCorporationFR      ETEKCITY Corporation (US)
            "23": {
                "subsidiary": 85,
                "entity": 811598
            }, // EtekcityCorporationIT     ETEKCITY Corporation (US)
            "20": {
                "subsidiary": 85,
                "entity": 811599
            }, //EtekcityCorporationUK     ETEKCITY Corporation (US)
            "11": {
                "subsidiary": 89,
                "entity": 811537
            }, // AdimanDE  ADIMAN B.V.
            ...
            "9": {
                "subsidiary": 87,
                "entity": 811553
            }, // ArovastUS  Arovast Corporation(US)
            "8": {
                "subsidiary": 85,
                "entity": 811605
            }, // EtekcityUS  Arovast Corporation(US)
            "27": {
                "subsidiary": 85,
                "entity": 811606
            }, // JP
        ...
        function getInputData() {
            ...
            var country = runtime.getCurrentScript().getParameter({
                name: 'custscript_country'
            });
            ...
            */
            ...
            core.amazon.getReportAccountList().map(function (acc) {
                ...
                search.create({
                    ...
                    var it = {
                        orderid: e.getValue(e.columns[0]),
                        acc: acc.id
                    };
                    orders.push(it);
                    return --limit > 0;
        ...
        function map(context) {
            ...
            try {
                ...
                // if (pr_store == 34 || pr_store == 35 || pr_store == 40) search_acc = EU_corr[pr_store] // HACK
                search.create({
                ...
                if (!so_id) {
                    if (orderid.length == 19 && pr_store && orderid.split("-")[0].length == 3) {
                        ...
                        // HACK 如果订单号不是正常的亚马逊单号   3-7-7
                        //如果此订单号为非正常的订单号：06fb53d9-3e9d-4c69-b424-64ba8a4f43fe,把此订单号的结算报告记录都搜出了标记为 F

                        // TODO 结算报告的字段 custrecord_missingorder_settlement 为创建
                        record.submitFields({
                            type: "customrecord_aio_amazon_settlement",
                ...
                var fils = [
                    ...
                    "and",
                    // ["custrecord_aio_sett_id", "is", "" + settle_id],    // HACK
                    // "and",
                ...
                PT_Arrys.map(function (pt) {
                    ...
                    // TODO  字段 custrecord_payment_itemprice_pt 未创建
                    record.submitFields({
                        type: "customrecord_aio_amazon_settlement",
                        id: pt,
                        values: {
                ...
                for (var key in settlmentID) {
                    context.write({
                        ...
                        value: {
                            ...
                            // "cl_date": m_postdate_obj[key.split("-")[1]][0], //按月分组的post date,取第一个就可以了 // HACK
