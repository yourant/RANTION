 ...
 * @LastEditTime   : 2020-09-14 19:54:45
 * @LastEditors    : Li
 * @Description    :
 * @FilePath       : \douples_amazon\dps_amazon_orderup_rl.js
...
define(['N/format', 'N/runtime', './Helper/core.min', './Helper/Moment.min', 'N/log', 'N/search',
    ...
    const _account = {
        ...
        "timezone": "5",
        "auth_meta": {
            "seller_id": "A67FFQ1POP4BF",
            "auth_token": "amzn.mws.e2b93500-3b94-0293-4599-c037feafed73",
            "access_id": "AKIAJ5PHZUW3JHGLMHUQ",
            "sec_key": "AS7wNA0GY7xxUKVFv0A5AswYClrRrrZh/kVxXb3f",
            "end_point": "mws.amazonservices.com"
        },
        "info": {
            "name": "KOHREE.US",
            "currency": "2",
            ...
            "salesorder_type": "",
            "salesorder_form": "68",
            "salesorder_location": "2",
            "salesorder_start_date": "",
            "dept": "4",
            "salesorder_payment_method": "",
            "discount_item": "",
            "shipping_cost_item": "",
            "tax_item": "6",
            "site_currency": "",
            "subsidiary": 5,
            "enable_tracking_upload": false,
            "enabled_tracking_upload_search": ""
        },
        "extra_info": {
            "if_fbaorder": true,
            "fbaorder_type": "",
            "fbaorder_form": "68",
            "fbaorder_location": "2664",
            "fbaorder_start_date": "",
            "if_including_fees": true,
            "if_handle_custrtn_report": false,
            "if_handle_removal_report": false,
            "if_handle_inventory_report": false,
            "if_handle_settlement_report": false,
            "to_default_from_location": "",
            "aio_to_default_form": ""
        },
        "marketplace": "ATVPDKIKX0DER",
        "preference": {
            "if_only_paid_orders": true,
            "salesorder_if_taxed": false,
            "salesorder_tax_rate_ipt": true,
            "salesorder_tax_auto_calc": true,
            "if_zero_price": false,
            "if_check_customer_email": true,
            "if_check_customer_addr": true,
            "if_payment_as_tran_date": true
        },
        "customer": "618",
        "enabled_sites": "Amazon US",
        "site_id": "1",
        "ord_formula": "{SKU售价}+{收取运费}-{促销折扣}-{运费折扣}+{giftwrap}"
    }
    ...
    function orderupRl(oid, cachid, acc) {
        var limit = 10 // 999 //350
        var deal_with_length = 0;
        var startT = new Date().getTime()
        log.debug('acc', acc)
        var orders = [],
            orderid
        ...
        log.debug('处理订单的数据长度 orders:' + orders.length)
        deal_with_length = orders.length;
        if (orders.length == 0)
            return '店铺 :' + acc + 'order:' + orders.length
        ...
        try {
            orders.map(function(obj) {
                ...
                log.audit('生成' + deal_with_length + '张单耗时:', new Date().getTime() - startT)
    ...
    function dealWithOrder(oid, cachid, acc) {
        var limit = 10 // 999 //350
        var deal_with_length = 0;
        var startT = new Date().getTime()
        log.debug('acc', acc)
        ...
        try {
            ...
            search.create({
                ...
                orders.push({
                    ...
                    id: _account.id,
                    currency: _account.info.currency,
                    auth: _account.auth_meta,
                    info: _account.info,
                    timezone: _account.timezone,
                    extra: _account.extra_info,
                    pref: _account.preference,
                    country: _account.country,
                    customer: _account.customer,
                    ord_formula: _account.ord_formula,
                    order: JSON.parse(rec.getValue('custrecord_aio_cache_body')),
                    iteminfo: rec.getValue('custrecord_amazonorder_iteminfo') ? rec.getValue('custrecord_amazonorder_iteminfo') : '',
                    enabled_sites: _account.enabled_sites
                })
                return --limit > 0
            })
    ...
    function serachDate(orderId, account, idForm) {
