 ...
 * @LastEditors    : Li
 * @LastEditTime   : 2020-04-14 16:38:35
 * @Description    : 
 * @FilePath       : \douples\dps_return_jour_mp.js
 * @Author         : Li
...
define(['N/search', 'N/record', './Moment.min', 'N/format', 'N/runtime'], function (search, record, moment, format, runtime) {
	...
	function getInputData() {
		...
		search.create({
			...
			filters: [
				{ name: 'custrecord_is_generate_voucher', operator: 'is', values: false },
		...
		*/
		...
		// 处理特定的数据
		var mySearch = search.load({ id: 87 });
	...
	function map(context) {
		var err = [];
		var order_id, trandate, entity, orderstatus, so_id, subsidiary, pr_store, currency_id, item_id;
		try {
			var rec_finance = record.load({ type: "customrecord_amazon_listfinancialevents", id: context.value })
			...
			search.create({
				...
				filters: [
					{ name: 'poastext', operator: 'is', values: rec_finance.getValue('custrecord_l_amazon_order_id') },
			...
			log.debug('info', { so_id: so_id, entity: entity, orderstatus: orderstatus, subsidiary: subsidiary, pr_store: pr_store, currency_id: currency_id, item_id: item_id })
			...
			if (so_id) {
				search.create({
					...
					filters: [
						...
						{ name: 'custbody_relative_finanace_report', operator: 'anyof', values: context.value },
				...
				var postdatetxt = rec_finance.getValue('custrecord_posteddate_txt');

				var fv = []
				var jour = record.create({ type: 'journalentry', isDynamic: true })

				jour.setValue({ fieldId: 'trandate', value: moment.utc(postdatetxt).toDate() })
				...
				fieldsMapping.map(function (field) {
					if (field == "LowValueGoodsTax-Principal" || field == "LowValueGoodsTax-Shipping") {
						...
						if (Number(rec_finance.getValue(financeMapping[field]))) {
							 ...
							 * 这个行的科目, 需要修改		125	
							 */
							jour.setCurrentSublistValue({ sublistId: 'line', fieldId: 'account', value: 125 })
							jour.setCurrentSublistValue({ sublistId: 'line', fieldId: 'memo', value: order_id })
							dr += Number(rec_finance.getValue(financeMapping[field]))
							jour.setCurrentSublistValue({ sublistId: 'line', fieldId: "credit", value: Number(rec_finance.getValue(financeMapping[field])).toFixed(2) }) //借 
							jour.commitLine({ sublistId: 'line' })
						}
					} else if (Number(rec_finance.getValue(financeMapping[field]))) {

						search.create({
							type: 'item', filters: [{ name: 'internalid', operator: 'anyof', values: item_id }],
						...
						if (incomeaccount) {
							...
							jour.setCurrentSublistValue({ sublistId: 'line', fieldId: 'memo', value: order_id })
							dr += Number(rec_finance.getValue(financeMapping[field]))
							jour.setCurrentSublistValue({ sublistId: 'line', fieldId: "credit", value: Number(rec_finance.getValue(financeMapping[field])).toFixed(2) }) ////贷
							jour.commitLine({ sublistId: 'line' })
						}
					}
				})
				...
				jour.setCurrentSublistValue({ sublistId: 'line', fieldId: 'memo', value: order_id })
				jour.setCurrentSublistValue({ sublistId: 'line', fieldId: "debit", value: Number(dr).toFixed(2) }) //借
				jour.setCurrentSublistValue({ sublistId: 'line', fieldId: "entity", value: entity }) //客户
				jour.commitLine({ sublistId: 'line' })
				var len = jour.getLineCount({ sublistId: "line" })
				log.debug("len::", len)
				jour.selectLine({ sublistId: 'line', line: len - 1 })
				var cmoun = jour.getCurrentSublistValue({ sublistId: "line", fieldId: "debit" })
				log.debug("0000cmoun:", cmoun)
				...
				rec_finance.setValue({ fieldId: 'custrecord_is_generate_voucher', value: true }) //预估计入已处理 
