 ...
 * @LastEditors    : Li
 * @LastEditTime   : 2020-04-13 20:35:23
 * @Description    : 
 * @FilePath       : \douples\dps_voucher_mp.js
 * @Author         : Li
	...
	function (search, record, moment, format, runtime) {
		...
		function getInputData() {
			...
			*/
			...
			// 处理特定的数据
			var mySearch = search.load({ id: 86 });
			...
			try {
				...
				search.create({
					...
					filters: [
						...
						{ name: 'custrecord_sku', operator: 'is', values: rec_finance.getValue('custrecord_sellersku') },
						{ name: 'custrecord_quantity_shipped', operator: 'is', values: rec_finance.getValue('custrecord_quantityshipped') },
						{ name: 'custrecord_amazon_order_item_id', operator: 'is', values: rec_finance.getValue('custrecord_l_amazon_order_id') }
					],
					columns: [
						{ name: 'custrecord_shipment_date_text' },
					]
				}).run().each(function (rec) {
					shipdate = rec.getValue("custrecord_shipment_date_text")
				})
				//搜索销售订单获取客户
				search.create({
				...
				if (so_id) {
					...
					jour.setValue({ fieldId: 'trandate', value: moment.utc(shipdate).toDate() })
