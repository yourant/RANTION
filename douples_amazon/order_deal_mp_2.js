  ...
  function (search, log, record, interfun, core) {
    function getInputData () {
      ...
      search.create({
        ...
        {name: 'custrecord23225',operator: 'is',values:false},
        ],columns: [
          {name: 'internalid'}
        ]
      }).run().each(function(e){
    ...
    function map (context) {
      try {
        ...
        var recs = record.load({type:"customrecord2019",id:context.value})
        var fulfill = record.load({type:"itemfulfillment",id:recs.getValue("custrecord23224")})
        var deks = fulfill.getValue("custbody_shipment_report_rel");
        search.create({
          type: 'invoice',
          filters: [
            { name: 'createdfrom', operator: 'anyof', values: deks},
        ...
        var de =  record.delete({type: 'itemfulfillment',id: recs.getValue("custrecord23224")});
        log.debug("de");
        if(inv_id.length>1) {
          log.audit("0000000，发货有问题",inv_id)
          return ;
        }
        var deso =  record.delete({type: 'customrecord_amazon_sales_report',id: deks});
        inv_id.map(function(ds){
          record.delete({type: 'invoice',id: ds}); 
        })
        record.submitFields({
          type: 'customrecord2019',
          id: context.value,
          values: {
            custrecord23225: true
          }
        })
