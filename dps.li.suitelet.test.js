 ...
 * @LastEditTime   : 2020-07-23 17:58:25
 * @LastEditors    : Li
 * @Description    : 
...
define(['N/search', 'N/record', 'N/log', './douples_amazon/Helper/core.min', 'N/file',
    ...
    function onRequest(context) {
        try {
            ...
            var account_id = 19,
                shipmentId = "FBA15P6KRNV2";

            var auth = core.amazon.getAuthByAccountId(account_id);

            var item = core.amazon.listInboundShipmentsItems(auth, shipmentId, "");
            ...
            context.response.writeLine(JSON.stringify(boxItem));
            ...
            context.response.writeLine(JSON.stringify(skuItem));
            context.response.writeLine("\n");

            boxItem.map(function (box) {
                var box_itemId = box.itemId;

                for (var i = 0, ilength = skuItem.length; i < ilength; i++) {

                    var temp = skuItem[i];

                    log.debug('box_itemId == temp.itemId', box_itemId + "  " + temp.itemId)

                    if (box_itemId == temp.itemId) {
                        box.sellersku = temp.sellersku
                    }
                }
            });
            ...
            context.response.writeLine(JSON.stringify(carton_infos_1));
            context.response.writeLine("\n");
            ...
            context.response.writeLine(body_1);
            context.response.writeLine("\n");
    ...
    function wms(af_rec) {
        ...
        if (token) {
            ...
            log.error('推送的数据', data);

            // 1 调拨单 2 销售出库单 3 退货出库 4 采购入库 5 退件入库
            // tool.wmsInfo(1752642, data, 1, '创建调拨单');
            // 发送请求
            // message = sendRequest(token, data);
        } else {
            message.code = 1;
